...
var levenDistCtrl = function levenDistCtrl($scope, $window) {
  ...
  $scope.spellCheck = function (wordToCheck) {
    ...
    for (i = 0; i <= $scope.englishWords.length - 1; i++) {
      var wordDist = $scope.calcLevenDistFly(wordToCheck, $scope.englishWords[i]);
